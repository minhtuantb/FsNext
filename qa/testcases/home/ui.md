...
| HOME-UI-009 | Phím ↑/↓ + Enter chọn kết quả | Overlay đang mở | 1) ↓ di chuyển highlight 2) Enter | Highlight di chuyển; Enter kích hoạt dòng đang chọn (mở file) thay vì submit | UI-driver | P2 | no | — |
...
| HOME-UI-011 | Quick-action điều hướng | Home | 1) Click từng card | "Dán link & tải"→ô tải, "Tải file lên"→Upload, "Thêm folder sync"→Sync, "Tạo link"→Files (đúng trang) | UI-driver | P2 | no | — |
