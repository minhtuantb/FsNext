...
> | H9 | ✅ FIXED (2026-05-29) | `FolderExpander` có depth-cap 20 + nay thêm `m_visited` cycle-detect (skip linkcode đã crawl) |
...
> M13 ✅ NOT-A-BUG (2026-05-29: `sortKey` chọn nhánh if/else cột cố định, `dir`=ASC/DESC từ bool — KHÔNG interpolate
> vào SQL, đã là whitelist) · M14 ✅ · M15 ✅ · M16 🔶 (đã có `requestedUserId` guard) · M17 ✅ (hash dedup) ·
> M18 ✅ FIXED (2026-05-29: tách walk filesystem ra `SyncScanner::scanFilesystem` chạy off-main, apply state ở main + guard reentrancy — phạm vi hẹp như plan, không refactor lõi sync) ·
> M19 ✅ (cap 50k) · M20 ⬜perf→BACKLOG · M21 ✅ FIXED (2026-05-29: `~SystemTray` delete m_menu — setContextMenu
> không sở hữu) · M22 ✅ (`main.cpp` shutdown-drain).
>
> **LOW:** L1 ✅ · L2 ✅ · L3 ✅ · L5 ✅notabug · L8 ✅ · L9 ✅ · L10 ✅ (còn lại: hygiene, không crash).
>
> Mục còn mở (không crash) → BACKLOG: H11, M3, M4, M13, M20, M21.
...
**Khắc phục (2026-05-29):** Tách phần đọc-filesystem thuần ra `SyncScanner::scanFilesystem()` (TU riêng, chỉ phụ
thuộc Qt6::Core — không FshareApi/TransferService/SQLite/QObject) chạy trên `QtConcurrent::run`; kết quả
(`ScanResult` POD) marshal về main qua `QMetaObject::invokeMethod` → `applyScanResult()` làm diff + persist
(`m_files`/`m_repo`/`m_createdSubdirs`) + `rebuildWatcher` + enqueue, TẤT CẢ ở main thread. Guard reentrancy
per-folder (`m_scanInFlight`/`m_scanDirty`) coalesce watcher/timer storms thành đúng 1 lần rescan dồn. Chữ ký
`scanFolderInternal` giữ nguyên → 11 callers không đổi (sync → async fire-and-forget). `ensureSubdirsThenEnqueue`
(network part) vốn đã async, không đụng. Unit test: [tests/test_sync_scan.cpp](tests/test_sync_scan.cpp).
